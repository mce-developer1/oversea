//modifed to createScorm 20060614 -- hao
//ref				<script	language="javascript" src="../../Scripts\Scorm_lms.js"></script>
//document.write('<script language="javascript" src="../../Scripts/APIWrapper.js"></script>');
//document.write('<script language="javascript" src="../../Scripts/SCOFunctions.js"></script>');
<!--
...
if (g_bInitializeOnLoad) {
	//alert("g_bInitializeOnLoad here");
	//loadPage();
	SCOInitialize_2();
}
...
function SCOInitialize() {
	//alert("#1 SCOInitialize");
	loadPage(); 
	timeIt();
	g_bInitDone = true;
}
function SCOInitialize_2() {
	// @@@ alert("#1 SCOInitialize_2");
	//loadPage(); return (true+"");	//sy: 20051020
	//doLMSInitialize(); return true();	//sy: 20051020
	var err = true;
	if (!g_bInitDone) {
...
function SCOFinish() {
	doLMSFinish(); return "true";	//sy: 20051020
...
function SCOGetValue(nam)		{return (doLMSGetValue(nam.toString()))}
function SCOCommit()			{return (doLMSCommit(""))}
function SCOGetLastError()		{return (doLMSGetLastError())}
function SCOGetErrorString(n)	{return (doLMSGetErrorString(n))}
function SCOGetDiagnostic(p)	{return (doLMSGetDiagnostic(p))}
...
function SCOSetValue(nam,val){
	doLMSSetValue(nam,val); return true;	//sy: 20051020
	var err = "";
	if (!APIOK()){
...
function privReportRawScore(nRaw) { // called only by SCOSetValue
	return true;    //sy: 20051020 to skip all at sDNA SCORM
...
function Down() {
   ...
   if (min == 0 && sec == 0) {
     if (g_bFinishDone == undefined) { 
       var msg = doLMSGetValue( "cmi.student_data.time_limit_action" );
       if (msg == "exit,message" || msg == "continue,message") {
         alert ("time-out");
       }
...
function timeIt() {
   //Get the max time from LMS. If value is empty, set value to default 10 mins.
...
function DoFSCommand(args){	
	//var tObj = g_bIsInternetExplorer ? MPHE3B : document.MPHE3B;	
	//var tObj = g_bIsInternetExplorer ?  document.all[sTitle]: document[sTitle];	//sy: document.[tArgs] ??
	// @@@ alert("Do Fscommand: (" + command + "," + args.substring(0,1024) + " ), tObj=" + tObj);
	//alert("doc.all:" + document.all[sTitle]);
	//alert("sTitle:" + document[sTitle]);
	// no-op if no SCORM API is available
	var myArgs = new String(args);
	//myArgs = unescape(myArgs);
	...
	if (sep > -1){
		cmd = myArgs.substr(0, sep);
		myArgs = myArgs.substr(sep+1);
		sep = myArgs.indexOf(",");
		if(sep > -1)
		{
		...
		cmd = myArgs;	//arg1 = myArgs;
		arg1 = "";
	}
	//if (!APIOK()) return;		//sy: 20051020 commented
	if (cmd.substring(0,3) == "LMS"){
		...
		if (cmd == "MM_cmiSendInteractionInfo") {
			...
			err = "false";	//SCOSetObjectiveData(F_intData[1], ".score.raw", F_intData[2])
			if (err=="true"){
		...
		if (cmd=="CMIFinish" || cmd=="CMIExitAU"){
			...
			if (false)	//eval('g_objAPI.' + cmd))		//sy: 20051020 ignored for now
			{
				err = eval('g_objAPI.' + cmd + '(\"' + arg1 + '\")');
			} else {
				err = "false"
	...
	if (false)	//(g_bShowApiErrors) && (err != "true"))	//sy: 20051020
	{
